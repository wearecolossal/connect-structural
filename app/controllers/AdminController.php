...
class AdminController extends \BaseController {
	...
	public function editProject($id)
	{
		...
		if(Input::hasFile('thumbnail')) {
			...
			$filename = 'proj-thumb-'.urlencode(strtolower(str_replace(':', '', str_replace(' ', '-', $input['name'])))).'-'.date('Y-m-d-H-i-s');
	...
	public function uploadPhoto($id)
	{
		...
		if(Input::hasFile('photo')) {
			...
			// generate a random file name
			$filename = 'proj-'.urlencode(strtolower(str_replace(':', '', str_replace(' ', '-', $p->name)))).'-'.date('Y-m-d-H-i-s');
			// assinged file input to a variable
			$image = $input['photo'];
			// get original image file extension
			$extension = $image->getClientOriginalExtension();
			...
			// generate a random file name
			$filename = 'proj-thumb-'.urlencode(strtolower(str_replace(':', '', str_replace(' ', '-', $p->name)))).'-'.date('Y-m-d-H-i-s');
			// assinged file input to a variable
			$image = $input['photo'];
			// get original image file extension
			$extension = $image->getClientOriginalExtension();
