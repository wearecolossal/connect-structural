        ...
        <div class="row" style="min-height:600px">
            @if(ProjectPhoto::where('project_id', $single->id)->first())
                <div class="col-md-7 proj-hero">
                    <img src="{{ URL::to('uploads') }}/{{ ProjectPhoto::where('project_id', $single->id)->orderby('id', 'asc')->first()->image }}"
                         alt=""/>
                </div>
            @endif
                ...
                <div class="proj-description">
                    ...
                    @if(ProjectPhoto::where('project_id', $single->id)->first())
                        <div class="row">
                            ...
                            <div class="col-md-6 proj-nav text-right"><a class="next_img">Next
                                    Image &rsaquo;</a><br/><br/></div>
							...
							@if($iterator == 1)
                                            active
                                            @endif
                                            project-photo" data-iterator="{{ $iterator++ }}"
                                                                           data-photo="{{ URL::asset('uploads/'.$photo->image) }}"><img
                                                src="{{ URL::asset('uploads/thumbnails/'.$photo->thumbnail) }}" alt=""/>
        ...
        <div class="row">
            ...
            @foreach(Project::where('category_id', $category->id)->where('archive', 0)->where('draft', 0)->orderby('name', 'asc')->get() as $project)
                <div class="project-list col-md-4">
                    <a href="{{ URL::to('projects/'.$category->id.'/'.urlencode(strtolower(str_replace(' ', '-', $category->name))).'/'.$project->id.'/'.urlencode(strtolower(str_replace(' ', '-', $project->name)))) }}"><img
                                src="{{ checkThumbnail($project->thumbnail) }}" alt=""/></a>
                    <h4>
                        <a href="{{ URL::to('projects/'.$category->id.'/'.urlencode(strtolower(str_replace(' ', '-', $category->name))).'/'.$project->id.'/'.urlencode(strtolower(str_replace(' ', '-', $project->name)))) }}">{{ $project->name }}</a>
                    </h4>
                </div>
                <?php
                $counter++;
        ...
        <div class="row">
            ...
            @foreach(Category::all() as $category)
                ...
                @if(Project::where('category_id', $category->id)->where('category_featured', 1)->first())
                    <div class="project-list col-md-4">
                        <a href="{{ URL::to('projects/'.$category->id.'/'.$category->url) }}"><img
                                    src="{{ checkThumbnail(Project::where('category_id', $category->id)->where('category_featured', 1)->first()->thumbnail) }}"
                                    alt=""/></a>
                        <h4>
                            <a href="{{ URL::to('projects/'.$category->id.'/'.$category->url) }}">{{ $category->name }}</a>
                        </h4>
                    </div>
                        <?php
                        $counter++;
                ...
                @else
                    <div class="project-list col-md-4">
                        <a href="{{ URL::to('projects/'.$category->id.'/'.$category->url) }}"><img
                                    src="{{ checkThumbnail(Project::where('category_id', $category->id)->where('archive', 0)->where('draft', 0)->orderby('created_at', 'desc')->first()->thumbnail) }}"
                                    alt=""/></a>
                        <h4>
                            <a href="{{ URL::to('projects/'.$category->id.'/'.$category->url) }}">{{ $category->name }}</a>
                        </h4>
